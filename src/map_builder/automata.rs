...
impl CellularAutomataArchitect {
    ...
    fn is_neighbor_wall(x: i32, y: i32, map: &Map) -> bool {
        let neighbor_idx = map_idx(x, y);

        map.tiles[neighbor_idx] == TileType::Wall
    }
    ...
    fn count_neighbors(&self, x: i32, y: i32, map: &Map) -> usize {
        ...
        for iy in -1..=1 {
            for ix in -1..=1 {
                let itself = ix == 0 && iy == 0;

                if !itself && CellularAutomataArchitect::is_neighbor_wall(x + ix, y + iy, map) {
                    neighbors += 1;
                }
            }
        }
